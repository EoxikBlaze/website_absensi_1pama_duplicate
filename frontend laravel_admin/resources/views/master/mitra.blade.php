    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 flex flex-col gap-4">
            <h3 class="font-extrabold text-slate-800 text-[15px] flex items-center gap-2">
                <i class="w-5 h-5 text-blue-500" data-lucide="list"></i> Daftar Mitra Subcontractor (<span id="totalMitra">{{ count($mitras) }}</span>)
            </h3>
                                ...
                                <td class="py-4 px-5 flex items-center justify-center">
                                    @if($mitra['latitude'] && $mitra['longitude'])
                                        <span class="bg-emerald-50 border border-emerald-100 text-emerald-600 font-extrabold text-[10px] px-2.5 py-1 rounded-md flex items-center gap-1">
                                            <i class="w-3 h-3" data-lucide="map-pin"></i> Terpasang
                                        </span>
                                    @else
                                        <span class="bg-red-50 border border-red-100 text-red-600 font-extrabold text-[10px] px-2.5 py-1 rounded-md flex items-center gap-1">
                                            <i class="w-3 h-3" data-lucide="x-circle"></i> Kosong
                                        </span>
                                    @endif
                                </td>
        ...
        <div class="lg:col-span-4 flex flex-col gap-4">
            <h3 class="font-extrabold text-slate-800 text-[15px] flex items-center justify-between">
                <span class="flex items-center gap-2" id="formTitle"><i class="w-5 h-5 text-emerald-500" data-lucide="plus-circle"></i> Tambah Mitra Baru</span>
            ...
            <form id="mitraForm" method="POST" action="{{ route('master.mitra.store') }}" class="bg-white p-6 rounded-2xl shadow-xl shadow-slate-200/40 border border-slate-100 relative overflow-hidden">
                @csrf
                ...
                <div class="flex flex-col mb-6 relative z-10">
                    ...
                    <input type="text" id="mitraNameInput" name="mitra_kerja_name" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl py-3 px-4 text-[13px] text-slate-800 font-bold outline-none focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all shadow-sm" required placeholder="Contoh: PT. BIMA PERSADA"/>
                </div>
                ...
                <div class="flex flex-col gap-3">
                    <button type="submit" id="btnSubmit" class="w-full px-6 py-3.5 bg-gradient-to-r from-[#0052cc] to-blue-600 hover:from-[#0047b3] hover:to-blue-700 text-white font-extrabold text-[12px] rounded-xl shadow-lg shadow-blue-500/30 transition-all active:scale-95 uppercase tracking-widest flex items-center justify-center gap-2">
                        <i class="w-4 h-4" data-lucide="save"></i> SIMPAN PERUSAHAAN
                    </button>
                    
                    <button type="button" id="btnDelete" onclick="deleteMitra()" class="hidden w-full px-6 py-3.5 bg-red-50 hover:bg-red-100 text-red-600 font-extrabold text-[12px] rounded-xl transition-all uppercase tracking-widest items-center justify-center gap-2 border border-red-100">
                        <i class="w-4 h-4" data-lucide="trash-2"></i> HAPUS PERUSAHAAN
                    </button>
                </div>
                
                <p class="text-[10px] text-slate-400 font-medium text-center mt-5">Untuk mengatur radius dan peta Geofence, silakan gunakan menu Geofence Lokasi secara terpisah.</p>
            </form>
        </div>
    </div>
</div>
...
<script>
    ...
    function editMitra(mitra) {
        currentEditId = mitra.mitra_kerja_id;
        form.action = `${updateUrlBase}/${mitra.mitra_kerja_id}`;
        methodField.value = 'PUT';
        
        mitraNameInput.value = mitra.mitra_kerja_name;
        
        formTitle.innerHTML = `<i class="w-5 h-5 text-blue-500" data-lucide="edit-3"></i> Edit: #${mitra.mitra_kerja_id}`;
        btnCancelEdit.classList.remove('hidden');
        
        btnSubmit.innerHTML = `<i class="w-4 h-4" data-lucide="save"></i> PERBARUI NAMA`;
    ...
    function resetForm() {
        ...
        formTitle.innerHTML = `<i class="w-5 h-5 text-emerald-500" data-lucide="plus-circle"></i> Tambah Mitra Baru`;
        btnCancelEdit.classList.add('hidden');
        
        btnSubmit.innerHTML = `<i class="w-4 h-4" data-lucide="save"></i> SIMPAN PERUSAHAAN`;
    ...
    function deleteMitra() {
        if(currentEditId && confirm("AWAS! Anda yakin menghapus Subcontractor ini?\nSeluruh data Geofence yang terkait dengan perusahaan ini akan ikut terhapus!")) {
